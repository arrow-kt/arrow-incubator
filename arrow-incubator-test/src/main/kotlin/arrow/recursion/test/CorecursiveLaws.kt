...
object CorecursiveLaws {
  ...
  private suspend fun <T, F> Corecursive<T, F>.anaEqualsHyloAndEmbed(coalg: Coalgebra<F, Int>, eqT: Eq<T>) =
    forAll(5, Arb.int().filter { it in 0..100 }) { i ->
      i.ana(coalg).equalUnderTheLaw(i.hylo(embed(), coalg, FF()), eqT)
    }

  private suspend fun <T, F> Corecursive<T, F>.apoEqualsAnaWithNormalCoalgebra(coalg: Coalgebra<F, Int>, eqT: Eq<T>) =
    forAll(5, Arb.int().filter { it in 0..100 }) { i ->
      i.apo {
  ...
  private suspend fun <T, F> Corecursive<T, F>.futuEqualsAnaWithNormalCoalgebra(coalg: Coalgebra<F, Int>, eqT: Eq<T>) =
    forAll(5, Arb.int().filter { it in 0..100 }) { i ->
      i.futu {
  ...
  private suspend fun <T, F> Corecursive<T, F>.anaMIsStackSafeWithEval(TF: Traverse<F>, coalg: CoalgebraM<F, ForEval, Int>) =
    5000.anaM(TF, Eval.monad(), coalg).value()

  private suspend fun <T, F> Corecursive<T, F>.apoMIsStackSafeWithEval(TF: Traverse<F>, coalg: CoalgebraM<F, ForEval, Int>) =
    5000.apoM(TF, Eval.monad()) {
      FF().run { coalg(it).map { it.map { it.right() } } }
    }.value()

  private suspend fun <T, F> Corecursive<T, F>.futuMIsStackSafeWithEval(TF: Traverse<F>, coalg: CoalgebraM<F, ForEval, Int>) =
